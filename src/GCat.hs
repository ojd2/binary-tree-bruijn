...
add x y |even_ x && even_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
...
add x y |even_ x && odd_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
  f EQ = leftshiftBy' (s a) (add as bs)
  f GT = leftshiftBy' (s b)
...
add x y | odd_ x && odd_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
  f EQ =  leftshiftBy'' (s a) (add as bs)
  f GT =  leftshiftBy'' (s b)
...
sub :: Cat a => a -> a -> a
sub x y | e_ y = x
sub x y | even_ x && even_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
...
sub x y | odd_ x && odd_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
  f EQ = leftshiftBy (s a) (sub  as bs)
  f GT = leftshiftBy (s b)
...
sub x y | odd_ x && even_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
  f EQ = leftshiftBy' (s a) (sub as bs)
  f GT = leftshiftBy' (s b)
...
sub x y | even_ x && odd_ y = f (comp a b) where
  (a,as) = c' x
  (b,bs) = c' y
...
comp :: Cat a=> a->a->Ordering
comp x y | e_ x && e_ y = EQ
comp x _ | e_ x = LT
comp _ y | e_ y = GT
comp x y | u_ x && u_ (s' y) = LT
comp  x y | u_ y && u_ (s' x) = GT

comp x y | x' /= y'  = comp x' y' where
  x' = bitsize x
  y' = bitsize y
...
comp xs ys =
  ...
  compBigFirst False False x y = f (comp a b) where
    (a,as) = c' x
    (b,bs) = c' y
    f EQ = compBigFirst True True as bs
    f LT = GT
    f GT = LT
  compBigFirst True True x y = f (comp a b) where
    (a,as) = c' x
    (b,bs) = c' y
...
rightshiftBy x y = f (comp x a')  where
  (a,b) = c' y
  a' = s a
  f LT = c (sub a x,b)
  f EQ = b
...
divAndRem :: Cat a => a -> a -> (a, a)
divAndRem x y | LT == comp x y = (e,x)
divAndRem x y | c_ y  = (q,r) where
  (qt,rm) = divstep x y
  (z,r) = divAndRem rm y
  q = add (exp2 qt) z
  ...
  tryToDouble x y k =
    if LT==comp x y then s' k
    else tryToDouble x (db y) (s k)

divide :: Cat b => b -> b -> b
divide n m = fst (divAndRem n m)

remainder :: Cat b => b -> b -> b
remainder n m = snd (divAndRem n m)
